...
def process_detecion(image, class_id, confidence, x, y, x_plus_w, y_plus_h):
    label = str(classes[class_id])
    print(label, class_id, confidence, x, y)
    if label == "car":
        print("Shuttle Detected!")
        requests.post("", json={"label": label, "confidence": confidence, "x": x, "y": y})
...
def detect(image):
    ...
    for i in indices:
        ...
        w = box[2]
        h = box[3]
        process_detecion(orgImage, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h))
